...
/// Upgrade an outbound [`rs_mojave_network_core::Transport::dial`] connection to a WebTransport connection.
pub(crate) async fn upgrade_outbound(
	...
	let fingerprint = if allow_tcp_fingerprint {
		...
		let fingerprint = hex::decode(response.text().await.map_err(Error::ReqwestError)?)
			.map_err(Error::HexError)?;
		Some(fingerprint)
	} else {
		None
	};
...
pub(crate) async fn read_public_key(
	...
	let bytes = framed.next().await.transpose()?.ok_or_else(|| {
	...
	let remote_public_key =
		libp2p_identity::PublicKey::try_decode_protobuf(&bytes).map_err(Error::Libp2pIdentity)?;
